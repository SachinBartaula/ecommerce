...
$recentOrders = [];
$recentResult = mysqli_query($conn, "SELECT orders.id, orders.total_amount, orders.status, orders.created_at, users.name AS customer FROM orders INNER JOIN users ON users.id = orders.user_id ORDER BY orders.created_at DESC LIMIT 6");
if ($recentResult) {
            ...
            <section class="min-w-0 flex-1">
                ...
                <div class="mt-6 grid grid-cols-1 gap-6 xl:grid-cols-3">
                    <div class="admin-card rounded-xl bg-white p-6 xl:col-span-2">
                        ...
                        <?php else: ?>
                            <div class="mt-5 overflow-x-auto"><table class="w-full min-w-[530px] text-left text-sm"><thead class="border-b border-slate-100 text-xs uppercase tracking-wide text-slate-400"><tr><th class="pb-3 font-semibold">Order</th><th class="pb-3 font-semibold">Customer</th><th class="pb-3 font-semibold">Date</th><th class="pb-3 text-right font-semibold">Total</th><th class="pb-3 text-right font-semibold">Status</th></tr></thead><tbody class="divide-y divide-slate-100"><?php foreach ($recentOrders as $order): ?><tr><td class="py-4 font-semibold text-slate-700">#<?php echo (int) $order["id"]; ?></td><td class="py-4 text-slate-600"><?php echo htmlspecialchars($order["customer"]); ?></td><td class="py-4 text-slate-500"><?php echo date("M j, Y", strtotime($order["created_at"])); ?></td><td class="py-4 text-right font-semibold text-slate-700">$<?php echo number_format((float) $order["total_amount"], 2); ?></td><td class="py-4 text-right"><span class="status-pill status-<?php echo htmlspecialchars($order["status"]); ?>"><?php echo htmlspecialchars($order["status"]); ?></span></td></tr><?php endforeach; ?></tbody></table></div>
                        <?php endif; ?>
                    </div>
